...
export default class Summarize extends Vue {
...
.summarize-container {
  ...
  $imgName: ('blue', 'green', 'pink', 'purple', 'medium-slate-blue');
  $imgExtension: '@2x.png';
  $fontColor: (#7ea1fd, #40e09c, #fe6da9, #9a7efd, #7e7efd);
  ...
  @each $item in $imgName {
    ...
    .icon- {
      &#{$item} {
        display: block;
        background: #fff url($imgFolder+'icon-'+$item+$imgExtension) no-repeat 0 0;
